...
- 外部查阅申请：一次申请可包含多个档案并逐份声明用途与动作，系统按密级分流（机密、绝密需人工审批，其余自动批准），支持部分批准、撤销申请和幂等创建（并发相同申请返回同一结果）。
- 查阅会话授权：批准后可为顾问开启有时效的会话，支持续期与提前撤销，离场后期限一到临时权限自动收回，过期或已撤销的会话不能再建立查阅下载记录。
- 查阅统计：管理员统计严格区分申请、批准与实际查阅三类事件。
